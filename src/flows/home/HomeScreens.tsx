...
import { ArrowDownToLine, ArrowLeftRight, Bell, Check, ChevronRight, CreditCard, ReceiptText } from "lucide-react";
    ...
    <AppShell>
      <div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        <div className="mt-8 grid gap-8 lg:grid-cols-[1.3fr_0.7fr]">
          <section aria-labelledby="activity-title"><div className="mb-4 flex items-center justify-between"><Heading id="activity-title" level={2}>Recent activity</Heading><Button variant="link" size="sm" asChild><Link to="/transactions">View all</Link></Button></div><Card><CardContent className="divide-y p-0">{activity.map((item) => <div key={item.id} className="flex items-center justify-between gap-4 p-4"><div><Text className="font-medium">{item.merchant}</Text><Text variant="small">{item.category} · {item.date}</Text></div><Text className={item.amount.startsWith("+") ? "font-medium text-primary" : "font-medium"}>{item.amount}</Text></div>)}</CardContent></Card></section>
          <section aria-labelledby="quick-actions-title"><Heading id="quick-actions-title" level={2} className="mb-4">Quick actions</Heading><div className="grid gap-3"><Button variant="outline" className="justify-start" asChild><Link to="/transactions/fund"><ArrowLeftRight aria-hidden="true" /> Add money</Link></Button><Button variant="outline" className="justify-start" asChild><Link to="/cards"><CreditCard aria-hidden="true" /> Manage cards</Link></Button><Button variant="outline" className="justify-start" asChild><Link to="/notifications"><Bell aria-hidden="true" /> Notifications</Link></Button><Button variant="outline" className="justify-start" asChild><Link to="/statement"><ReceiptText aria-hidden="true" /> Download statement</Link></Button></div></section>
        </div>
      </div>
    </AppShell>
  );
